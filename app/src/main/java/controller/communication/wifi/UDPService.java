...
public class UDPService extends SurService {
    ...
    public void startServer(final Context context, NetworkDiscovery callback, final ErrorInterface errorCallback) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    socket.send(packet);
                    final DatagramPacket packRecv = new DatagramPacket(buffer, buffer.length);
                    new Thread(new Runnable() {
                        @Override
                        public void run() {
                            try {
    ...
    public void sendConnect(final int port, final InetAddress adress, final ErrorInterface errorInterface) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                try {
